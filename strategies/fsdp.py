...
from typing import (TYPE_CHECKING, Any, Callable, ContextManager, Dict,
                    Generator, List, Literal, Optional, Set, Tuple, Type,
                    Union)
...
from lightning.fabric.strategies.strategy import (
    TBroadcast, _apply_filter, _BackwardSyncControl, _Sharded,
    _validate_keys_for_strict_loading)
from lightning.fabric.utilities.distributed import (
    ReduceOp, _distributed_is_initialized,
    _get_default_process_group_backend_for_device, _init_dist_connection,
    ...
    def load_checkpoint(
        ...
        if _is_sharded_checkpoint(path):
            ...
            with state_dict_ctx:
                ...
                _distributed_checkpoint_load(module_state, path, strict=strict)
            ...
            metadata = torch.load(path / _METADATA_FILENAME, weights_only=False)
            ...
            for key in requested_metadata_keys:
                if key not in metadata:
                    continue
                if isinstance(state[key], DataLoader):
                    state[key].load_state_dict(metadata.pop(key))
                    # print(key)
        ...
        if _is_full_checkpoint(path):
            ...
            from lightning.fabric.strategies.model_parallel import (
                _load_raw_module_state, _rekey_optimizer_state_if_needed)
...
def _distributed_checkpoint_load(
    module_state: Dict[str, Any], path: Path, strict: bool
) -> None:
    if _TORCH_GREATER_EQUAL_2_3:
        ...
        load(
            ...
            planner=DefaultLoadPlanner(
                flatten_sharded_tensors=False, allow_partial_load=(not strict)
            ),
        )
    else:  # deprecated
        from torch.distributed.checkpoint import FileSystemReader
        ...
        reader = FileSystemReader(path=path)
        load(module_state, reader)  # Copyright The Lightning AI team.
