...
try:
    from models.fa3_integration import flash_attention_3_forward

    ALL_ATTENTION_FUNCTIONS.update({"flash_attention_3": flash_attention_3_forward})
except ModuleNotFoundError:
    print("FA3 not found : skipping import.")
    pass
...
class LlamaRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6, scaling_factor=20):
        """
        ...
        if self.weight.dtype in ("bfloat16", torch.bfloat16):
            self.weight.data = self.weight.data / self.scaling_factor
...
class LlamaRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            pos_id_device = position_ids_expanded.device
            freqs = (
                inv_freq_expanded.to(pos_id_device).float()
